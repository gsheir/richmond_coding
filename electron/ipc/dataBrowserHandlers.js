...
export function registerDataBrowserHandlers({ registerHandler, getDatabase }) {
  registerHandler('list-tables', async () => {
    const tables = getDatabase().listTables();
    return { success: true, tables };
  }, { requireDatabase: true });

  registerHandler('get-table-schema', async (_event, tableName) => {
    const schema = getDatabase().getTableSchema(tableName);
    return { success: true, schema };
  }, { requireDatabase: true });

  registerHandler('get-table-data', async (_event, tableName, options) => {
  ...
  registerHandler('get-row-count', async (_event, tableName, filters) => {
    const count = getDatabase().getRowCount(tableName, filters || {});
    return { success: true, count };
  }, { requireDatabase: true });

  registerHandler('get-related-data', async (_event, tableName, rowId) => {
    const related = getDatabase().getRelatedData(tableName, rowId);
    return { success: true, related };
  }, { requireDatabase: true });

  registerHandler('update-table-row', async (_event, tableName, rowId, columnUpdates) => {
    const updated = getDatabase().updateTableRow(tableName, rowId, columnUpdates);
    return { success: true, updated };
  }, { requireDatabase: true });

  registerHandler('delete-table-row', async (_event, tableName, rowId) => {
    const deleted = getDatabase().deleteTableRow(tableName, rowId);
    return { success: true, deleted };
  }, { requireDatabase: true });

  registerHandler('delete-table-rows', async (_event, tableName, rowIds) => {
    const deletedCount = getDatabase().deleteTableRows(tableName, rowIds);
    return { success: true, deletedCount };
  }, { requireDatabase: true });

  registerHandler('insert-table-row', async (_event, tableName, rowData) => {
